...
BAUD = 57600  # WindMaster standard baud rate
...
def pick_mode():
    ...
    print("  1) Continuous until configuration command or Ctrl+C")
...
def main():
    ...
    print(f"WindMaster Simulator (Mode 10: Binary UVW Long)")
    ...
    if count_limit:
        ...
        print("Mode: continuous until configuration command or Ctrl+C")

    print("\nWaiting for initialization sequence:")
    print("  1. '*' command to enter configuration mode (will echo 'CONFIGURATION MODE')")
    print("  2. 'Q' command to return to measurement mode (starts binary output)")
    print("\nPacket structure:")
    print("  - header: 0xB4B4")
    ...
    try:
        while True:
                ...
                for ch in text:
                    if ch.isspace() or ch == '\n' or ch == '\r':
                        if cmd_buf:
                            ...
                            if token == "*":
                                # Enter configuration mode - stop output
                                in_config_mode = True
                                started = False
                                print("[CMD] '*' → CONFIGURATION MODE (output stopped)")
                                # Echo back response
                                ser.write(b"CONFIGURATION MODE\r\n")

                            elif token == "Q":
                                # Exit configuration mode - start measurement mode
                                if in_config_mode:
                                    in_config_mode = False
                                    started = True
                                    sent_count = 0
                                    next_send = time.monotonic()
                                    print("[CMD] 'Q' → MEASUREMENT MODE (binary output started)")
                                    # Echo back response (just 'Q' acknowledgment)
                                    ser.write(b"Q\r\n")
                                else:
                                    # Already in measurement mode, ignore
                                    pass
            ...
            now = time.monotonic()
            if started and not in_config_mode and now >= next_send:
